...
struct TestContext {
    ...
    explicit TestContext(
        ...
        : logic(std::move(decisions)),
          orchestrator(definitionStore, executionStore, stepExecutionStore, logic) {
...
WorkflowStepExecution claimStartStep(TestContext& context, const std::string& workerId = "worker-001") {
...
TEST_CASE("orchestrator start creates workflow execution and initial pending step") {
    ...
    const auto stepExecution = context.stepExecutionStore.find(
...
TEST_CASE("orchestrator poll-and-claim claims a pending step with a lease") {
    ...
    const auto storedStep = context.stepExecutionStore.find(
        execution.workflowExecutionId,
        "validateOrder",
        0
    );
...
TEST_CASE("orchestrator complete step with active lease advances to next pending step") {
    ...
    const auto completedStep = context.stepExecutionStore.find(
        execution.workflowExecutionId,
        "validateOrder",
        0
    );
    ...
    const auto nextStep = context.stepExecutionStore.find(
        execution.workflowExecutionId,
        "chargePayment",
        0
    );
...
TEST_CASE("orchestrator rejects complete after lease expiry") {
    ...
    auto claimedStep = context.stepExecutionStore.find(
        execution.workflowExecutionId,
        "validateOrder",
        0
    );
...
TEST_CASE("orchestrator fail step with active lease creates retry pending step") {
    ...
    const auto failedStep = context.stepExecutionStore.find(
        execution.workflowExecutionId,
        "validateOrder",
        0
    );
    ...
    const auto retryStep = context.stepExecutionStore.find(
        execution.workflowExecutionId,
        "validateOrder",
        1
    );
